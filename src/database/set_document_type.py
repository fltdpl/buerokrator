from src.database.database import open_connection


def set_document_type(document_id, document_type):
    """Ändert den Dokumenttyp und setzt das Dokument auf ungeprüft zurück.

    Bewusst nicht über update_document: die verlangt den vollen Feldsatz und
    benennt die Datei um. Nach einem Typwechsel gelten andere Felder, das
    Dokument gehört erneut in den Prüf-Workflow.
    """
    with open_connection() as conn:
        cursor = conn.cursor()

        cursor.execute(
            """
            UPDATE documents
            SET document_type = ?, verified = 0
            WHERE id = ?
            """,
            (document_type, document_id),
        )

        conn.commit()
