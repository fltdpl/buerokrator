...
def import_page():
    with page_layout("Import"):
        ui.label("📥 Dokument importieren").classes("text-2xl font-bold")

        # ------------------------------------------------------------
        # Stapel-Import
        # ------------------------------------------------------------
        ui.label("📦 Stapel-Import").classes("text-xl font-bold")
        ui.label(
        ...
        def import_area():
            ...
            if not pending:
                ui.label("Keine Dateien im Inbox-Ordner.").classes("text-gray-500")
                return

            ui.label(f"{len(pending)} Datei(en) im Inbox-Ordner:")

            for path in pending:
                ui.label(f"• {path.name}").classes("text-sm text-gray-500")
            ...
            async def start_import():
                ...
                with results_column:
                    ui.label(f"✅ {len(succeeded)} Dokument(e) archiviert.").classes(
                        "text-green-700"
                    )
        ...
        # ------------------------------------------------------------
        ui.label("⬆️ Dateien hochladen").classes("text-xl font-bold")
        ui.label("Legt die Dateien in den Inbox-Ordner für den Stapel-Import.").classes(
            "text-gray-500"
        )
