...
def test_fremder_anbieter_behaelt_seinen_aussteller():
    foreign = INLINE_STATEMENT.replace("Musterbau\nBausparkasse AG", "Zweitbau Bausparkasse AG")
    llm = {"issuer": "Zweitbau Bausparkasse AG", "product_name": "Bausparvertrag"}

    refined = refine_pension_fields(foreign, llm)

    assert refined["issuer"] == "Zweitbau Bausparkasse AG"
...
def test_steuerbescheinigung_ist_anbieterunabhaengig():
    ...
    broker = TAX_CERTIFICATE.replace("Musterbau\nBausparkasse AG", "Beispielbank eG")
...
def test_refine_korrigiert_verrutschte_llm_werte():
    ...
    assert refined["issuer"] == "Musterbau"
